...
何謂 TRANSACTION
  ...
  完成TRANSACTION的學習後，逼需再修正之前的stock的修改、刪除的功能，加上TRANSACTION的處理
      stock table的修改與刪除部分
        {MODULE updel_stock.4gl}
        GLOBALS "stores_globs.4gl"

        FUNCTION updel_init()
          DECLARE lockstock CURSOR FOR
            SELECT * FROM stock
              WHERE stock.stock_num = gt_stock.stock_num AND
                    stock.manu_code = gr_stock.manu_code
          FOR UPDATE
        END FUNCTION
          ...
          ELSE
            MENU "Are you sure you want to delete this record?"
              ...
              COMMAND "YES" "Delete this record."
                ...
                ELSE
                  IF SQLCA.SQLCODE < 0 THEN
                    ERROR "Row could not be locked for delete."
                    ROLLBACK WORK
                  ELSE
                    WHENEVER ERROR CONTINUE
                    DELETE FROM stock
        ...
        FUNCTION update_stock()
          ...
          BEGIN WORK
          OPEN lockstock
          WHENEVER ERROR CONTINU
          FETCH lockstock INTO gr_stock.*
          WHENEVER ERROR STOP
          IF SQLCA.SQLCODE < 0 OR SQLCA.SQLCODE = NOTFOUND
            THEN
            ERROR "Could not lock row for update."
            ROLLBACK WORK
          ELSE
        --在此使用ur_stock.*存放使用者更改資料前的欄位值
        --以利後續若使用者按CONTROL-C 放棄修改時，可將原始資料
        --再顯示於螢幕上，該變數於stores_globs.4gl中宣告
            ...
            ELSE
              ...
              UPDATE stock
                SET (description, unit_price, unit, unit_descr)
                  = (gr_stock.description THRU gr_stock.unit_descr)
                WHERE CURRENT OF lockstock
              EHENEVER ERROR STOP
              IF SQLCA.SQLCODE < 0 THEN
                ERROR "Error No.", SQLCA.SQLCODE
  ...
  注意事項：
      使用FOR UPDATE CURSOR指令時，若此DATABASE有transaction logging，必須在transaction中去OPEN及CLOSE 此CURSOR。如果在DECLARE時使用WITH HOLD，就可以在transaction之外去OPEN及CLOSE此CURSOR，雖然如此，FETCH動作還是必須在transaction中進行
      如果在DECLARE CURSOR時加上WITH HOLD，可避免此cursor_name因為一個TRANSACTION的結束(結束一個TRANSACTION可能由下達ROLLBACK或COMMIT WORK指令而引起)而被CLOSE掉

  stock table 的查詢DECLARE CURSOR部分 加上WITH HOLD
    {MODULE brws_stock.4gl}
    GLOBALS "stores_globs.4gl"
    ...
    FUNCTION query_stock()
      DEFINE where_clause CHAR(250),
              sql_qry CHAR(300),
              stock_cnt INTEGER   ///將符合條件之總筆數存放於此變數
      ...
      --以下部分為挑選符合條件的資料
      LET sql_qry = "SELECT * FROM stock WHERE",
                    where_clause CLIPPDE
      PREPARE ex_stmt1 FROM sql_qry
      DECLARE stock_ptr SCROLL CURSOR WITH HOLD FOR   ///查詢DECLARE CURSOR部分 加上WITH HOLD
        ex_stmt1
      ...
      LET sql_qry = "SELECT COUNT (*) FROM stock WHERE",
                      where_clause CLIPPED
      OPEN stock_ptr
      FETCH FIRST stock_ptr INTO gr_stock.*
        IF SQLCA.SQLCODE = NOTFOUND THEN
          ERROR "No rows found."
          CALL clean_up()
          RETURN FALSE
        ELSE
          CALL display_stock()
          PREPARE ex_stmt2 FROM sql_qry
          DECLARE stock_count CURSOR FOR ex_stmt2
          OPEN stock_count
          FETCH stock_count INTO sotck_cnt
          CLOSE stock_count
          FREE stock_count
          MESSAGE "There are", stock_cnt USING "<<<<",
                  "rows selected."
    ...
    Stores_globs.4gl 加上DEFINE ur_stock.*的宣告
      {MODULE_1, stores_globs.4gl}
...
撰寫REPORT FUNCTION
  REPORT FUNCTION的寫法
      定義一個REPORT的程式區塊，如FUNCTION

      REPORT report_name (參數一, 參數二,...) 
        [DEFINE Section]
        [OUTPUT Section]
        [ORDER BY Section]
        FORMAT Section
      END REPORT
      ///report_name即START REPORT或OUTPUT TO REPORT的report_name()中的參數對應到OUTPUT TO REPORT report_name(4GL運算式,...)中的4GL運算式，個數需一樣，資料型態須相呼應
      ///REPORT FUNCTION中參數可以是常數或變數，不能有 .*符號, 陣列或使用THRU；如是變數，因屬區域變數，需進一步利用DEFINE Section定義
      ///REPORT FUNCTION由四個Section組成，如上述語法所示，但只有FORMAT Section才須具備

      REPORT simple(x)
              DEFINE x RECORD LIKE customer.*
              FORMAT EVERY ROW
      END REPORT
          ///在此傳遞x參數，利用DEFINE Section，進一步定義該參數的型態是與 customer table一致

      REPORT cust_list(pr_cust)

  DEFINE Seciton
      定義變數，與INFORMIX 4GL程式中的DEFINE variable為一樣的方式
      以下狀況參數須被傳遞，且須在此宣告
          使用FORMAT EVERY ROW產生預設的報表，每筆紀錄的所有值都必須傳遞
          在REPORT FUNCTION中含ORDER BY Section，則ORDER BY 後的變數的值也需要傳遞
          如使用AFTER GROUP OF control block，則在AFTER GROUP OF 後的變數值也需傳遞
          如使用AGGREGATE FUNCTION，且此AGGREGATE FUNCTION並不是被使用在ON LAST ROW control block中，則()中的變數值也需傳遞

  OUTPUT Section
      此Section主要在訂定報表的頁面配置集中端輸出媒體
        
        {以下為 OUTPUT Section的語法}
        OUTPUT
          [REPORT TO 子句]
          [頁面配置]
        以下為[REPORT TO 子句] 的詳細語法
        REPORT TO
          {SCREEN | PRINTER | [FILE] file_name} |
        ...
        LEFT MARGIN [=] #
        TOP OF PAGE [=] "string"
          ///如不需要訂定報表的頁面配置終端輸出媒體，此Section可省

        OUTPUT
              REPORT TO  "cust_list.out"
              TOP MARGIN 0
              BOTTOM MARGIN 0
              PAGE LENGTH 6
  ...
  ORDER BY Section
      此Section主要在設定input records的排序方式，也連帶決定在FORMAT Section中GROUP OF control block的順序

      ORDER [EXTERNAL] BY 變數_1 [ASC | DESC]   ///預設為ASC；如果有重定義ORDER BY Section，加上EXTERNAL則不會重複執行SORT
        [, 變數_2[ASC | DESC],...]

      REPORT cust_list(cust)
              DEFINE cust RECORD LIKE customer.*
              ORDER BY cust.company

      ORDER EXTERNAL BY customer_num

  FORMAT Section
      制定報表的輸出格式
      4GL支援兩種FORMAT Section一種為簡單式(FORMAT內部只有EVERY ROW)，一種為複雜式(在FORMAT內部視需要加入各種Control Block)

      FORMAT   ///組合二：此行加上行3到行9的一個或更多的子句
          EVERY ROW   ///組合一：EVERY ROW 加此行
          [AFTER GROUP OF 子句]   
          [BEFORE GROUP OF 子句]   
          [FIRST PAGE HEADER 子句]    ///只能使用一次
          [PAGE HEADER 子句]   
          [ON EVERY ROW 子句]   
          [PAGE TRAILER 子句]   
          [ON LAST ROW 子句]  ///只能使用一次

        Control Block被執行時機
        FIRST PAGE HEADER：在第一頁開始時
        PAGE HEADER：在每一頁開始時
        BEFORE GROUP OF：在已經索引過的資料，且按照某欄位群組，當每一群組開始被處理前
        ON EVERY ROW：當每一筆記錄被送到REPORT時
        AFTER GROUP OF：在已經索引過的資料，且按照某欄位群組，當每一群組結束被處理後
        PAGE TRAILER：在每一頁要結束時
        ON LAST ROW：在最後一筆紀錄被REPORT處理後
        Not Any：如果START REPORT與FINISH REPORT間沒有任何資料記錄被送出，則Control Block不會被執行

    REPORT cust_list (customer)
        DEFINE customer RECORD LIKE customer.*
        FORMAT
          EVERY ROW
    END REPORT

    REPORT orders_list(order_num, customer_num, order_date)
        DEFINE customer_num LIKE orders.customer_num,
        DEFINE order_num LIKE orders.order_num,
        DEFINE order_date LIKE orders.order_date
        FORMAT
          EVERY ROW
    END REPORT

  AFTER GROUP OF Control Block
    主要訂定當目前GROUP 結束時，要完成的事項

    AFTER GROUP OF variable_name
      指令...
    ///variable_name必須為input record中的一個參數名稱
    ///下列三種時機該Control Block會被執行：  
                  此variable值被改變時
                  更高階層的GROUP的值改變時
                  在input record 最後一筆資料被輸出，但4GL執行ON LAST ROW或是PAGE TRAILER之前
    ///使用此Control Block一定要將資料記錄按該欄位排序過，排序方式有二
                  SELECT資料時即下達ORDER BY 指令，但是在REPORT FUNCTION中也要使用ORDER EXTERNAL BY Section
                  SELECT資料時不排序，但在REPORT FUNCTION中使用ORDER BY Section
                  在AFTER GROUP OF Control Block中將GROUP置於PRINT指令中，並與AGGREGATE FUNCTION(AVG,SUM,MIN,MAX)合用
      AFTER GROUP OF orders.order_num
        PRINT"", orders.order_date, 7 SPACES, orders.order_num
          USING "###&", 8 SPACES, orders.ship_date, "",
          GROUP SUM (orders.total_price)
          USING "$,$$$,$$$.&&"

  BEFORE GROUP OF Control Block
      該部分只要訂定一個新的GROUP開始前要完成的事項
      BEFORE GROUP OF variable_name
        指令...

      ///variable_name必須為input record中的一個參數名稱
      ///下列三種時機該Control Block會被執行
        AFTER GROUP OF Control Block完成執行後
        更高階層的GROUP的值改變時
        BEFORE GROUP OF control block的執行權高於 ON EVER ROW
      ///使用此Control Block一定要將資料記錄該欄位排序過
               SELECT資料時即下達ORDER BY 指令，但是在REPORT FUNCTION中也要使用ORDER EXTERNAL BY Section
               SELECT資料時不排序，但在REPORT FUNCTION中使用ORDER BY Section
      BEFORE GROUP OF orders.customer_num
        SKIP TO TOP OF PAGE
  
  FIRST PAGE HEADER Control Block
      主要用在input record的第一筆開始動作前，作用為制定該報表的第一頁的表頭
      FIRST PAGE HEARDER 指令...

      ///由於此部分被執行的時機是產生任何輸出之前，所以可以利用此部分訂定變數的初值，亦可利用此部分訂定變數的初值
      ///也可利用此部分制定報表的名稱及各欄位標題
      ///若沒有任何input record，則該部分不會被執行
      ///如果此部分與PAGE HEADER同時並存，則第一頁會以FIRST PAGE HEADER為主，其他頁才是以PAGE HEADER為主
      ///此指令有限制
            不能使用 SKIP # LINES 於迴圈指令中
            不能使用 NEED
            如果使用IF... THEN ... ELSE ... END IF，則THEN中的PRINT 列數必須與ELSE中的PRINT列數一樣
            如果再CASE, FOR 或 WHILE中使用PRINT，則每個PRINT的結束均須是；符號
            不可使用PRINT FILE
      FIRSH PAGE HEADER
          LET i=1

      FIRST PAGE HEADER
          PRINT COLUMN 30, "客戶清單"
          SKIP 2 LINES
          PRINT "客戶所在州別為：", cond_state
          SKIP 1 LINE
          PRINT "客戶編號", COLUMN 12, "姓名", COLUMN 35, "所在地", COLUMN 65, "電話"
          SKIP 1 LINE

  PAGE HEADER Control Block
      PAGE HEADER主要制定該報表的每一頁的表頭
      PAGE HEADER 指令...
      ///此部分被執行的時機是每頁的表頭，可利用此部分制定報表的個欄位標題或頁碼
      ///也可將GROUP的欄位置於此部分列印，因4GL確保此部分執行時的欄位值會與ON EVERY ROW的欄位值相同
      ///如此部分與PAGE HEADER為主，其他頁則以PAGE HEADER為主
  ...
  ON EVERY ROW
      該部分主要在制定對每一筆input record的動作

      ON EVERY ROW指令...

      此部分被執行的時機是任何一筆 input record 被送達到 REPORT FUNCTION

      ON EVERY ROW
          IF (city IS NOT TULL) AND (state IS NOT NULL) THEN
  ...
  PAGE TRAILER Control Block
      ...
      此部分被執行的時機是在每新頁確定產生，要執行PAGE HEADER之前，下列狀況均可能產生新頁：
          PRINT 的指令執行而一頁的列數滿時
          執行到 SKIP TO TOP OF PAGE指令
          SKIP # LINES執行時遇到一頁的列數滿時
          NEED指令執行時遇到一頁的列數滿時
  ...
  ON LAST ROW Control Block
      主要在制定當4GL處理完input record的最後一筆後所要採取的動作

      ON LAST ROW 指令...

      ///此部分被執行的時機是當最後一筆input record已完成 ON EVERY ROW及 AFTER GROUP OF動作後
      ///此部分通常會使用AGGREGATE FUNCTION已顯示一些TOTAL的資訊

    ON LAST ROW
        SKIP 1 LINE
        PRINT COLUMN 12, "Total Number of Customers:", COLUMN 57, COUNT(*) USING "#&"


FORMAT Section中的指令
  REPORT FUNCTION部分，能用的指令只有EXIT REPORT, NEED, PAUSE, PRINT, SKIP

  EXIT REPORT
      此指令會中止REPORT的處理程序並返回到OUTPUT TO REPORT指令的下一行指令

      EXIT REPORT

      ///此指令會中止目前報表的處理程序
      ///刪除任何中間過程的檔案或是暫時的TABLE

  NEED
      幫助偵測本頁的剩餘列印行數是否足夠接下來欲列印區段的總列數，若偵測結果不夠時，會執行跳頁動作

      NEED # LINES

      ///#是程式設計師所填入的一正整數，通常會先評估接下來欲列印區段的總列數，然後填入#中
      ///如果剩餘列數少於#，系統會執行SKIP TO TOP OF PAGE(跳頁指令)

    AFTER GROUP OF orders.order_num
        NEED 6 LINES
        PRINT "", orders.order_date, 7 SPACES,
            GROUP SUM(orders.total_price)
                  USING "$,$$$,$$$.&&"

  PAUSE
      若終端輸出媒體為SCREEN時，可以使用此指令讓資料在顯示滿一畫面時暫停，直到使用者按ENTER後才繼續下一畫面之輸出

      PAUSE["一段敘述"]

      PAGE TRAILER
          SKIP 1 LINE
          PAUSE "Press ENTER to display next screen."

  PRINT
      輸出內容，是FORMAT Section的核心

      {PRINT指令可配合使用的狀況較多，列示如下}
      PRINT
        [4GL 運算式]   ///可以是常數、變數或是一個運算結果
        [COLUMN #]   ///表示由左邊界開始算起的第#個字元位置，使數字不能超過(右邊界-左邊界)
        [PAGENO]   ///提供目前頁次
        [LINENO]   ///提供目前頁次之第幾列
        [BYTE 變數]   ///為DATA TYPE是BYTE的變數
        [整數運算式 SPACE | SPACES] ///SPACE | SPACES指多少個字元空間，通常在訂定每個欄位之間的空距
        [AGGREGATE REPORT FUNCTION]
        [字元運算式 | 文字變數             ///若是LONG CHARACTER的資料，可以設定WORDWRAP， 使得資料在輸出時能夠以連續的方式處理
            [WORDWRAP] [RIGHT MARGIN #]]       通常設定WORDWRAP時，會另外在指定暫時RIGHT MARGIN，如不指定則內定為132 CHARACTERS
        [FILE "filename"]   ///可指定一個文字檔在此列印
        ///4GL 運算式 到 FILE "filename" 間要用逗點，或分號；隔開
        ///[AGGREGATE REPORT FUNCTION] 再說明，其語法可再詳述如下：
        [GROUP]
        {PERCENT(*) | COUNT(*) |AVG(數值運算式|區間運算式)|
          SUM(數值運算式|區間運算式)|
          MAX(4GL 運算式) | MIN(4GL 運算式)}
        [WHERE BOOLEAN 運算式]

      ///每一個PRINT均會造成印字頭移動到下一列，如果是用分號；則會接續目前列繼續列印

      列印時須考慮每個欄位所需的位元數
            INTEGER：11      BYTE：12    FLOAT：14         SMALL FLOAT：14      SMALLINT：6    DATE：10             
            DECIMAL：(2+m)，m為宣告時的precision   MONEY： (3+m)，m為宣告時的precision
            INTERVAL：3-25，視資料欄位所宣告之長度  DATATIME：2-25，視資料欄位所宣告之範圍 
            CHAR：視資料欄位所宣告之長度       VARCHAR：視資料欄位所宣告之長度      TEXT：視資料欄位所宣告之長度
        ...
        AFTER GROUP OF number
              ...
              PRINT 4 SPACES, "Count of small orders: ",
                COUNT(*) WHERE total_price <2000.00
                  USING "##.###"
              SKIP 1 LINE
              PRINT 5 SPACES, "Total amount for the order：",
                  Ship_charge + GROUP SUM(total_price)
                  USING "$$,$$$,$$$.&&"
          ///此處的GROUP SUM(total_price)會以number為群組，加總相同number的items的資料記錄中的total_price

        PRINT COLUMN 10, resume WORDWRAP RIGHT MARGIN 75
        ...
        SELECT * FROM customer, cnt
        ...
        ///以上為準備資料階段，由於cnt 為一個暫時的TABLE，所以當SELECT資料從兩個TABLE時可以不需要寫JOIN條件
        FORMAT
          FIRST PAGE HEADER
            LET y = cnt/30 --假設每頁列印30筆
          PAGE TRAILER
        ...
        FIRST PAGE HEADER
          ...
          PRINT "客戶所在州別為：", cond_state
          SKIP 1 LINE
          PRINT "客戶編號", COLUMN 12, "姓名", COLUMN 35,
                "所在地", COLUMN 65, "電話"
          SKIP 1 LINE
        ON EVERY ROW
          PRINT customer_num USING "###&", COLUMN 12,
                fname CLIPPED, 1 SPACE, lname CLIPPED,
                COLUMN 35, city CLIPPED, ",", state,
                COLUMN 57, zipcode, COLUMN 65, phone

        PRINT "Dear", 1 SPACE, "fname", ","
        PRINT FILE "/usr/mary/occupant.txt"

  SKIP
    該指令主要插入空列或是跳至下一頁

    SKIP(TO TOP OF PAGE) | # (LINE | LINES)
    ///SKIP TO TOP OF PAGE會強迫跳至下一頁，此指令不可用在FIRST PAGE HEADER、PAGE HEADER或PAGE TRAILER這些Control Block
    ///#唯一正整數，表要在目前列印處插入#空白列，不可用在CASE, FOR, WHILE中
...
FORM中的SCREEN ARRAY
  ...
  資料記錄不重複的ATTRIBUTES Section
    ATTRUBITES
      ...
      f007=customer.phone
      ...此部分為資料記錄重複的程式碼
      f011=item5.item_num NOENTRY;
      f012=items.stock_num;
      ...
      f016=items.total_price, NOENTRY;
      END

    將下述的程式片段組合起來完成此order_form的程式碼
      INSTRUCTIONS
      SCREEN RECORD s_items[5](item_num, stock_num, manu_code, description, quantity, unit_price, total_price)


4GL的INPUT ARRAY
  DEFINE ARRAY VARIABLE
    定義陣列變數以供程式中使用
    DEFINE variable_name ARRAY[#1, #2, #3] OF
      4GL簡單資料型態|
      RECORD 資料型態|
      BYTE | TEXT

    DEFINE ga_stock ARAY[5] OF RECORD LIKE stock.*
  ...
  LET
    設定陣列變數的值
      LET variable_name[#]{.member}=NULL
      LET variable_name[#]{.member}=常數|變數|運算表示式
        此陣列變數定義時為RECORD所組成，{}表可省略
        該語法主要將變數設定為常數、變數或一個運算式

    LET p_array[2]=2
    LET p_items[1].item_num=1101
    LET p_items[1]=NULL


  CLEAR
    將螢幕中的陣列值清掉
    CLEAR variable_name[#].*|.member
      .*此陣列中所有成員
      .member可以指定特定的成員
    DEFINE i SMALLING
    FOR i=1 TO 4
    CLEAR s_items[i].*  ///SCREEN FORM中定義的陣列組紀錄
    END FOR

  INPUT ARRAY
    主要提供在INFORMIX-4GL程式中對於陣列變數，藉由螢幕陣列變數輸入資料存放到程式中的陣列變數

    INPUT ARRAY program_array [WITHOUD DEFAULTS]  ///加上WITHOUT DEFAULTS時，program array的值不會被清除掉，通常是用在修改資料的功能時
          FORM screen_array.*  ///為SCREEN FORM撰寫 INSTRUCTIONS Section 所定義的陣列變數，
                                 上一行及本行合起來就是將使用者經由螢幕所輸入的陣列資料轉放至程式中的陣列變數
          [HELP 輔助說明之編號]
          [ATTRIBUTE 子句]  ///一部分與之前學習的一樣，另一部分是屬於ARRAY特有的(CURRENT 到 DELETE 五行)
        [input control block...
          END INPUT]
      有關以上 [ATTRIBUTE 子句] 部分在此加入選擇性語法
    CURRENT ROW DISPLAY = "KEYWORD,..."  ///可以進一步指定目前游標所在的陣列紀錄該筆顯示時的特效，KEYWORD可以式顏色或是強度的特效  
                                            (但不可為DIM, INVISIBLE, NORMAL)
    COUNT = # | variable_name  ///#為正整數, variable的型態必須是INT或SMALLINT，而且其值必須是正整數，
                                該子句主要告知CURRENT SCREEN的陣列中有資料的筆數，COUNT=5與CALL SET_COUNT(5)具有相同意義
    MAXCOUNT = #|variable_name  //////#為正整數, variable的型態必須是INT或SMALLINT，而且其值必須是正整數，
                                該子句主要限制CURRENT SCREEN中的陣列筆數的數目
    INSERT ROW = TRUE|FALSE  ///可設定使用者在INPUT ARRAY能否使用INSERT KEY來新增資料，為FALSE則不能使用INSERT KEY新增資料
    DELETE ROW = TRUE |FALSE  //////可設定使用者在INPUT ARRAY能否使用DELETE KEY來刪除資料，為FALSE則不能使用DELETE KEY新增資料
      有關以上[input control block...在此描述
      其中使用{} 並有 | 該符號者，
      表示被刮柱部分為其中的選項
    BEFORE | AFTER
      {{FIELD(Field List) | INPUT | DELETE| INSERT |ROW} |   ///Field List中的field必須是SCREEN ARRAY
        ON KEY(按鍵，...)}
    {4GL statement |NEXT FIELD
          {(Field List) |NEXT |PREVIOUS}
    | {EXIT | CONTINUE} INPUT}
    ///BEFORE INPUT block
          觸發時機(trigger time)
              在顯示預設的資料後但使用者被允許開始輸入資料前；
          使用時機為顯示訊息給使用者以告知如何使INPUT ARRAY，或是一些初始值的設定與顯示

    ///BEFORE ROW block
          此處的ROW為一筆SCREEN RECORD
          觸發時機  
              當游標移動到SCREEN FORM的一個新列時  或
              由於空間不夠所引起的INSERT指令失敗 或
              按了INTERRUPT 或是QIUT鍵後所引起的INSERT指令中斷  或
              使用者按了 DELETE KEY

    ///BEFORE DELETE block
          觸發時機
              當使用者按了DELETE KEY去刪除SCREEN FORM上的陣列記錄後，但INFORMIX-4GL尚未去執行DATABASE內的實際刪除記錄動作前
          使用時機：在USER選了該筆螢幕資料記錄要做刪除後，但資料庫中一些規則不允許此筆資料被刪除時，可以在此阻止該種不正常的刪除發生
          建議此時使用EXIT INPUT或是CANCEL DELETE，且能夠提供訊息給使用者

    ///BEFORE INSERT block
          觸發時機
              當使用者開始輸入一筆新的記錄到此陣列時
              當使用者按了INSERT KEY要去新增資料在SCREEN FORM上的陣列記錄，不過還沒新增進去時
              使用者移動游標到最後面開始另一空白螢幕陣列時
          使用時機：通常此時可能會取得或設定一些欄位的值，並將其顯示出來
          可以使用CANCEL INSERT，取消使用者的新增工作

    ///AFTER INSERT block
          觸發時機
              當使用者新增一筆記錄到此螢幕陣列後，將游標移動到下一個新的螢幕陣列記錄時
          INPUT ARRAY子句中只能有一個AFTER INSERT block

    ///AFTER DELETE block
          觸發時機
              當使用者按DELETE KEY刪除一筆螢幕陣列記錄後，若AFTER ROW block存在，則會執行AFTER ROW block後再執行之
          INPUT ARRAY子句中只能有一個AFTER DELETE block    

    ///AFTER ROW block
          此處的ROW，指的是螢幕記錄
          觸發時機
              當使用者移動游標離開目前的螢幕記錄時，通常是因為使用者按下列任一按鍵
                  任何一個箭頭按鍵
                  ENTER 或 TAB
                  ACCEPT KEY
                  INTERRUPT KEY
                  INSERT KEY
          若AFTER ROW block 與 AFTER INSERT block均存在時，則4GL執行完 AFTER INSERT block會馬上執行AFTER ROW block
          INPUT ARRAY子句中只能有一個 AFTER ROW block

    ///AFTER INPUT block
          觸發時機
              當使用者按下列任一按鍵，而引起結束INPUT ARRAY指令但並沒有結束4GL程式的執行
                  ACCEPT KEY
                  INTERRUPT KEY
                  QUIT KEY
          通常會在此block使用GET_FLDBUF()或是FIELD_TOUCHED()這些內建函數去檢查、改變或是存入使用者輸入的值，通常也會與NEXT FIELD一併使用

    ///END INPUT只有在使用input control block後才需要加入此部分，而且要在最後一個control block後

    ///有幾個與INPUT ARRAY或 DISPLAY ARRAY常一起使用的內建函數

    例句
        INPUT ARRAY p_items FROM s_items.*

        INPUT ARRAY p_items FROM s_items.* HELP 301

        INPUT ARRAY p_addr FROM s_addr.* ATTRIBUTE (GREEN, REVERSE)

        INPUT ARRAY p_items FORM s_items.*
                ATTRIBUTE(WHITE, CURRENT ROW DISPLAY= "RED, REVERSE")

        INPUT ARRAY p_items WITHOUT DEFAULTS FROM s_items.*
                ATTRIBUTE(INSERT ROW=FALSE, DELETE ROW = FALSE)

        INTPUT ARRAY p_items FROM s_items.*
                BEFORE INPUT DISPLAY "Press ESC to enter data" AT 22,1
        ...
        INPUT ARRAY p_items WITHOUT DEFAULTS
                ...
                BEFORE INPUT
                  ...
                  DISPLAY p_items[pa_curr].sock_num TO
        ...
        BEFORE INSERT
                CALL get_item_num()
                  ///FUNCTION ger_item_num()程式碼如下
                        {該FUNCTION主要是取得item_num的序號}
        ...
        AFTER INSERT OF items
                CALL renum_items()
                    ///FUNCTION renum_items()的程式碼如下
                      {該FUNCTION主要將item_num的序號重新排列}
        ...
        INPUT ARRAY p_items FROM s_items.*
              ...
              IF p_items[pa_curr].manu_code = "PNG" THEN
                  MESSAGE
                      "NOTE:PNG products are currently on hold"
                  NEXT FIELD manu_code
              END IF
        END INPUT
        ///該例句同時有AFTER ROW及NEXT FIELD，雖然是目前螢幕記錄出問題，但其游標將會移動到下一筆記錄的此欄位；所以此狀況建議使用AFTER INPUT block

        學習完INPUT ARRAY的指令後，就可以完成order form中有關INPUT ARRAY的部分，如下所示
            {此部分定義城市中的陣列變數}
            DEFINE p_items[10] OF RECORD
              item_num      LIKE items.item_num,
              ...
              total_price   LIKE items.total _price
            END RECORD
            {此部分設定陣列的初值}
            DEFINE i SMALLINT
            ...
            {此部分設定一個SCROLLING CURSOR由TABLE中取得資料，經由此CURSOR將資料送進城市中的陣列變數，再利用INPUT ARRAY將資料送到螢幕陣列以供使用者輸入資料}
            DECLARE q_curs CURSOR FOR
              SELECT item_num, items.stock_num, items.manu_code, description, quantity, unit_price, total_price
                FROM items, stock
                WHERE order_num = p_order_num AND
                      items.stock_num = stock.stock_num AND
            ...
            INPUT ARRAY p_items WITHOUT DEFAULTS FROM s_items.*
            {接下來在INPUT ARRAY 中加入一些CONTROL BLOCK}
              BEFORE FIELDS stock_num
                MESSAGE "Enter a stock No."
              AFTER FIELD stock_num, manu_code
                ...
                END IF
            END INPUT
            {FUNCTION get_item()主要在取得對弈的description與unit_price}
            FUNCTION get_item()
              DEFINE pa_curr, sc_curr SMALLINT
              LET pa_curr = ARR_CURR()
              LET sc_curr = SCR_LINE()
              ...
              FROM stock
                WHERE stock.stock_num = p_items[pa_curr].stock_num AND
                      stock.manu_code = p_items[pa_curr].manu_code
              DISPLAY p_items[pa_curr].description, p_items[pa_curr].unit_price
                TO s_items[sc_curr].description, s_items[sc_curr].unit_price
            END FUNCTION

            {FUNCTION get_total() 主要在計算 quantity * unit_price}
            FUNCTION get_total()
              DEFINE pa_curr, sc_curr SMALLINT
              LET pa_curr = ARR_CURR()
              LET sc_curr = SCR_LINE()
              ...
              DISPLAY p_items[pa_curr].total_price
                  TO s_items[pa_curr].total_price
            END FUNCTION


4GL的DISPLAY ARRAY 與內建函數
  DISPLAY ARRAY
      主要是將程式中的陣列變數顯示在SCREEN FORM上的陣列變數

      DISPLAY ARRAY program_array TO screen_array.*  ///screen array.* 為SCREEN FORM撰寫 INSTRUCTIONS Section定義的陣列變數，
                                                        主要將程式中的陣列變數的值搬給螢幕陣列變數以顯示出來
            [ATTRIBUTE 子句]
            [ON KEY block]
            [END DISPLAY]
        --ATTRIBUTE 子句，在此加入其他選擇性語法
      CURRENT ROW DISPLAY = "keyword,..."  ///可以進一步指定目前游標所在的陣列紀錄該筆顯示時的特效，keyword可以式顏色或強度的特效
                                              ，但不能為DIM, INVISIBLE, NORMAL
        --[ON KEY block block...]，在此描述
      ON KEY(按鍵，...)  ///參考下列能進一步指定按某一按鍵時能結束DISPLAY ARRAY
      [4GL 或 SQL statement | EXIT DISPLAY] 
      ///[END DISPLAY] 子句 是選擇性的，但有下列句子時一定要使用 [END DISPLAY] 子句
          DISPLAY ARRAY子句中使用 [ON KEY block]
          DISPLAY ARRAY子句被使用在CONSTRUCT, INPUT, INPUT ARRAY子句中，且ON KEY block又緊跟在後，卻屬於CONSTRUCT, INPUT, INPUT ARRAY 子句中的 ON KEY
          DISPLAY ARRAY子句被使用在PROMPT子句中時或是在另一個DISPLAY ARRAY子句中

      DISPLAY ARRAY p_items to s_items.*

      DISPLAY ARRAY p_items to s_items.*
              ATTRIBUTE (WHILE, CURRENT ROW DISPLAY = "GREEN, REVERSE")
  ...
  內建函數
    以下函數於使用INPUT ARRAY或 DISPLAY ARRAY時常會利用之函數
    ARR_CURR():傳回取得4GL的程式變數陣列中目前所在的第幾筆
    ARR_COUNT()：傳回4GL的程式變數陣列中目前有資料的總筆數
    SCR_LINE()：傳回SCREEN FORM中的螢幕陣列中目前所在的第幾筆
    SET_COUNT()
        取得目前程式變數陣列中有資料的總筆數，會將此取得值設定做為ARR_COUNT()的初始值
        此用INPUT ARRAY WITHOUT DEFAULTS 或是 DISPLAY ARRAY 時必須先使用此函數
    FIELD_TOUCHED()
        判斷使用者是否更改過()中指定的欄位值，如改過則傳回TRUE
    GET_FLDBUF()：傳回一個或多個欄位的內容，為字串值
    FGL_GETKEY()：等待使用者按一按鍵，且會將此按鍵相對的一個整數值傳回來
    FGL_LASTKEY()：會將使用者最近按的一按鍵的相對應的整數值傳回來
    INFIELD()：判斷目前欄位名稱是否為()中所指定的欄位名稱，如是傳回TRUE

    接下來完成有關顯示陣列資料的部分，主要是顯示state table中的資料

    {此功能主要利用ARRAY在一個畫面上多筆顯示state table的資料}
    DATABASE stores7
    GLOBALS
      DEFINE ga_state ARRAY[50] OF RECORD LIKE state.*
    END GLOBALS

    MAIN 
      DEFINE stat_cnt SMALLINT
      DEFER INTERRUPT
      CALL load_state() RETURNING state_cnt
      IF state_cnt > 0 THEN
        CALL state_wind (state_cnt)
      END IF 
    END MAIN

    {以下的FUNCTION load_state()，設定一SCROLLING CURSOR由資料庫中取出資料記錄放入程式陣列變數中；並取得state table中目前有多少筆記錄}
    FUNCTION load_state()
      DEFINE state_cnt SMALLINT
      DECLARE cur_state CURSOR FOR
        SELECT * FROM state ORDER BY code
      LET state_cnt=1
      FOREACH cur_state INTO ga_state[state_cnt].*
    ...
    {以下FUNCTION state_wind(state_cnt) 將程式中的陣列變數放入SCREEN FORM中的陣列變數，並顯示出}
    FUNCTION state_wind(state_cnt)
      DEFINE state_cnt SMALLINT
      OPEN WINDOW w_state_wnid AT 3,40
        WITH FORM "state_wind" ATTRIBUTE (BORDER)
      DISPLAY "Use Arrow Keys to Move Cursor." AT 1,1
      CALL set_count(state_cnt)
      DISPLAY ARRAY ga_state TO sa_state.*

      CLOSE WINDOW w_state_wind
    END FUNCTION